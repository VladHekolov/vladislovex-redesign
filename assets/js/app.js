...
/* Mobile reviews use a native, looping scroll track instead of the desktop 3D scene. */
(function () {
  'use strict';
  ...
  function prepareImage(item, logicalIndex) {
    ...
    if (!image.getAttribute('src') && deferred) image.src = deferred;
    image.loading = logicalIndex < 2 ? 'eager' : 'lazy';
    image.decoding = 'async';
    image.setAttribute('fetchpriority', logicalIndex === 0 ? 'high' : 'low');
    function loaded() {
    ...
    image.addEventListener('error', function () {
      ...
      var fallback = /\.jpe?g([?#].*)?$/i.test(source)
        ? source.replace(/\.jpe?g(?=([?#].*)?$)/i, '.png')
        : /\.png([?#].*)?$/i.test(source)
          ? source.replace(/\.png(?=([?#].*)?$)/i, '.jpg')
          : '';
      if (!fallback) {
        item.classList.add('is-image-error');
        return;
  ...
  function makeItem(source, index, cloneSide) {
    var item = source.cloneNode(true);
    item.removeAttribute('id');
    item.querySelectorAll('[id]').forEach(function (child) { child.removeAttribute('id'); });
    item.removeAttribute('style');
    item.dataset.vhReviewLogical = String(index);
    if (cloneSide) {
      item.dataset.vhLoopClone = cloneSide;
      item.setAttribute('aria-hidden', 'true');
    }
    prepareImage(item, index);
    return item;
  }
  ...
  sourceItems.forEach(function (item, index) { scene.appendChild(makeItem(item, index, '')); });
  ...
  function itemLeft(index) {
    ...
    return item ? Math.max(0, item.offsetLeft - Math.max(0, (wrap.clientWidth - item.offsetWidth) / 2)) : 0;
  }
  function scrollPhysical(index, smooth) {
    currentPhysical = Math.max(0, Math.min(index, physicalItems.length - 1));
    if (smooth) wrap.scrollTo({ left: itemLeft(currentPhysical), behavior: 'smooth' });
    else {
      var behavior = wrap.style.scrollBehavior;
      wrap.style.scrollBehavior = 'auto';
      wrap.scrollLeft = itemLeft(currentPhysical);
      requestAnimationFrame(function () { wrap.style.scrollBehavior = behavior; });
    }
  ...
  function settle() {
    ...
    physicalItems.forEach(function (item, index) {
      var next = Math.abs(item.offsetLeft + item.offsetWidth / 2 - center);
      if (next < distance) {
        distance = next;
        nearest = index;
      }
    });
    if (nearest < cloneCount) nearest += originalCount;
    if (nearest >= cloneCount + originalCount) nearest -= originalCount;
    scrollPhysical(nearest, false);
  }
  ...
  wrap.addEventListener('scroll', function () {
    clearTimeout(settleTimer);
    settleTimer = setTimeout(function () { settle(); schedule(5200); }, 150);
  }, { passive: true });
  ...
  wrap.addEventListener('touchmove', function (event) {
    var touch = event.touches && event.touches[0];
    if (touch && (Math.abs(touch.clientX - touchStartX) > 8 || Math.abs(touch.clientY - touchStartY) > 8)) suppressOpen = true;
  }, { passive: true });
  ...
  scene.addEventListener('click', function (event) {
    if (!suppressOpen) openItem(event.target.closest('.vh-reviews-3d__item'));
  });
  scene.addEventListener('keydown', function (event) {
    if (event.key === 'Enter' || event.key === ' ') {
      event.preventDefault();
      openItem(event.target.closest('.vh-reviews-3d__item'));
    }
  });
...
(function () {
  function initMobileBenefitsCarousel() {
    if (window.innerWidth > 860) return false;

    var oldRail = document.getElementById('vhBenefitsRail');
    if (!oldRail || oldRail.dataset.vhLoopReady === 'true') return false;
  ...
  if (initMobileBenefitsCarousel()) return;
...
(function () {
  ...
  function showMobilePdfReady(blob, filename) {
    ...
    modal.innerHTML =
      '<div class="vh-mobile-pdf-ready__card" role="dialog" aria-modal="true" aria-label="PDF готов">' +
        ...
        '<p>Откройте PDF или сохраните его через меню «Поделиться».</p>' +
  ...
  function saveOfferPdf(pdf, filename) {
    var safeName = filename || 'vystuplenie-muzykanta-vladislav-hekolov.pdf';
    if (isMobileViewport() && isIOSDevice()) {
      showMobilePdfReady(pdf.output('blob'), safeName);
      return;
    }
  ...
  function downloadOfferPdf() {
    ...
    loadPdfLibraries().then(function () {
      ...
      saveOfferPdf(pdf, 'vystuplenie-muzykanta-vladislav-hekolov.pdf');
